...
export function LeadsPage() {
  ...
  const columns = useMemo(
    () => [
      ...
      columnHelper.display({
        id: 'actions',
        header: 'Actions',
        cell: ({ row }) => <LeadRowActions onOpen={() => setSelectedLead(row.original)} />,
      }),
    ],
    [],
  )
  ...
  const industryFilter =
    (table.getColumn('industry')?.getFilterValue() as string | undefined) ?? 'All'
    ...
    <div className="space-y-6">
      ...
      <SectionCard className="space-y-4">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          <label className="flex items-center gap-2 rounded-xl border border-slate-700 bg-slate-900/70 px-3 py-2 text-sm text-slate-400">
            <Search className="h-4 w-4" />
            <input
              value={globalFilter}
              onChange={(event) => setGlobalFilter(event.target.value)}
              placeholder="Search company, contact, or industry..."
              className="w-full bg-transparent text-slate-200 outline-none placeholder:text-slate-500 sm:w-80"
            />
          </label>
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            >
              <option>All</option>
              {Array.from(new Set(mockLeads.map((lead) => lead.industry))).map((industry) => (
          ...
          <table className="w-full min-w-[760px] text-left text-sm">
            ...
            <tbody>
              {table.getRowModel().rows.map((row) => (
                <tr
                  key={row.id}
                  className="cursor-pointer border-b border-slate-800/80 transition-colors hover:bg-slate-900/40 last:border-none"
                  onClick={() => setSelectedLead(row.original)}
                >
                  {row.getVisibleCells().map((cell) => (
                    <td
                      key={cell.id}
                      className={`px-4 py-3 text-slate-200 ${(cell.column.columnDef.meta as { className?: string } | undefined)?.className ?? ''}`}
                    >
